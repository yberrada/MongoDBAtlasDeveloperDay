...
### Step 1: Access MongoDB Atlas cluster  
   ...
   - Double click on the project listed to access your Atlas Deployment.   ![alt text](https://github.com/yberrada/MongoDBAtlasDeveloperDay/blob/main/public/readme/project.png) 
...
Once the index is ready, go back to `Collections`. Select the `wikipedia` collection and go to the `Aggregations` tab like before. Hit `Add Stage` and enter `$search` for the first stage. Atlas should provide you with the syntax.
...
```
    {
        "$search": {
            "knnBeta": {
                /**
                * The vector value below is the vectorized version of the following string: "When was Automatic Payrolls founded?".
                */
